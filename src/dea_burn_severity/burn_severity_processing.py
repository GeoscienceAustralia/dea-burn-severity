...
def process_single_fire(
    ...
    if not fire_date:
        fallback_raw = _first_valid_value(
    ...
    start_date_pre = (
        ...
        - timedelta(days=burn_config.pre_fire_buffer_days)
    ).strftime("%Y-%m-%d")
    end_date_pre = (
    ...
    if extinguish_date == "None":
        start_date_post = (
            datetime.strptime(fire_date, "%Y-%m-%d")
            + timedelta(days=burn_config.post_fire_start_days)
        ).strftime("%Y-%m-%d")
    else:
    ...
    end_date_post = (
        ...
        + timedelta(days=burn_config.post_fire_window_days)
    ).strftime("%Y-%m-%d")
    ...
    severity = calculate_severity(delta_nbr, landcover, burn_config.grass_classes)
    ...
    base_dir = out_dir if out_dir else burn_config.output_dir
...
def _format_results_filename(
    ...
    identifier = (
        clean_fire_slug(str(identifier_source).strip())
...
def _build_vector_filename(
    ...
    if identifier_src in (None, ""):
        identifier_src = _first_valid_value(fire_series, burn_config.FIRE_ID_FIELDS)
    processed_date_src = attributes.get("date_processed")
    if processed_date_src in (None, ""):
        processed_date_src = _first_valid_value(
...
def _extract_attribute_values(series: pd.Series) -> dict[str, Any]:
    ...
    for rule in burn_config.ATTRIBUTE_COPY_RULES:
...
def process_all_polygons(all_polys: gpd.GeoDataFrame) -> None:

    os.makedirs(burn_config.output_dir, exist_ok=True)
    print(f"All outputs will be saved to: {burn_config.output_dir}")

    upload_to_s3 = burn_config.upload_to_s3
    s3_prefix = burn_config.upload_to_s3_prefix
    s3_fs = None
    if upload_to_s3:
        try:
            import s3fs  # type: ignore
    ...
    for idx, fire_series in all_polys.iterrows():
        ...
        if not burn_config.force_rebuild:
